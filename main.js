...
function fUnload(callback) {
    try {
        ...
        Object.keys(timeoutplays).forEach(timeout => {
            clearTimeout(timeoutplays[timeout]);
        });
        Object.keys(timeoutstates).forEach(timeout => {
            clearTimeout(timeoutstates[timeout]);
        });
        Object.keys(timeoutstarted).forEach(timeout => {
            clearTimeout(timeoutstarted[timeout]);
        });
    ...
    if (id && state && !state.ack) {
        ...
        switch (cmd) {
            case 'command.toggleplay':
                request.post(
                    {
                        uri: `http://${adapter.config.ip}/Sessions/${dId}/Playing/PlayPause?api_key=${
            ...
            case 'command.play':
                request.post(
                    {
                        uri: `http://${adapter.config.ip}/Sessions/${dId}/Playing/Unpause?api_key=${
                            adapter.config.apikey
                        }`,
                        headers: headers,
                    },
            ...
            case 'command.pause':
                request.post(
                    {
                        uri: `http://${adapter.config.ip}/Sessions/${dId}/Playing/Pause?api_key=${
                            adapter.config.apikey
                        }`,
                        headers: headers,
                    },
            ...
            case 'command.mute':
                request.post(
                    {
                        uri: `http://${adapter.config.ip}/Sessions/${dId}/Command/Mute?api_key=${
                            adapter.config.apikey
                        }`,
                        headers: headers,
                    },
            ...
            case 'command.unmute':
                request.post(
                    {
                        uri: `http://${adapter.config.ip}/Sessions/${dId}/Command/Unmute?api_key=${
                            adapter.config.apikey
                        }`,
                        headers: headers,
                    },
            ...
            case 'command.togglemute':
                request.post(
                    {
                        uri: `http://${adapter.config.ip}/Sessions/${dId}/Command/ToggleMute?api_key=${
                            adapter.config.apikey
                        }`,
                        headers: headers,
                    },
            ...
            case 'command.goHome':
                request.post(
                    `http://${adapter.config.ip}/Sessions/${dId}/Command/GoHome?api_key=${adapter.config.apikey}`,
                    {},
                    function (error, resp, _body) {
                        if (error) {
                            adapter.log.info(`Fehler: ${JSON.stringify(resp)}`);
            ...
            case 'command.goToSearch':
                request.post(
                    `http://${adapter.config.ip}/Sessions/${dId}/Command/GoToSearch?api_key=${adapter.config.apikey}`,
                    {},
                    function (error, resp, _body) {
                        if (error) {
                            adapter.log.info(`Fehler: ${JSON.stringify(resp)}`);
            ...
            case 'command.back':
                request.post(
                    `http://${adapter.config.ip}/Sessions/${dId}/Command/Back?api_key=${adapter.config.apikey}`,
                    {},
                    function (error, resp, _body) {
                        if (error) {
                            adapter.log.info(`Fehler: ${JSON.stringify(resp)}`);
            ...
            case 'command.volume':
                adapter.log.info(
                    `http://${adapter.config.ip}/Sessions/${dId}/Command/SetVolume?api_key=${adapter.config.apikey}`,
                );
                request.post(
                    {
                        uri: `http://${adapter.config.ip}/Sessions/${dId}/Command/SetVolume?api_key=${
                            adapter.config.apikey
                        }`,
...
function webMessage(e) {
    ...
    for (let i = 0; i < data.Data.length; i++) {
        ...
        if (
            adapter.config.deviceIds == '' ||
            (adapter.config.deviceIds != '' && adapter.config.deviceIds.indexOf(d.Id) == -1)
        ) {
            createDevice(d);
            if (typeof d.DeviceName !== 'undefined') {
                adapter.setState(`${d.Id}.info.deviceName`, d.DeviceName, true);
            } else {
                adapter.setState(`${d.Id}.info.deviceName`, '', true);
            }
            ...
            if (typeof d.NowPlayingItem !== 'undefined') {
                ...
                if (d.NowPlayingItem.RunTimeTicks != null && d.PlayState.PositionTicks != null) {
                    const endDate = new Date(
                        Date.now() + (d.NowPlayingItem.RunTimeTicks - d.PlayState.PositionTicks) / 10000,
                    );
                    endString = `${endDate.getHours()}:${
